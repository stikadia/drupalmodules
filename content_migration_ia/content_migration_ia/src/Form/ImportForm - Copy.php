...
class ImportForm extends FormBase{
	...
	protected $msgObj, $filesystem, $entityTypeManager, $file_url_generator;
	...
	public function __construct() {
		...
		foreach ($languages as $language) {
			$this->allowlangs[] = $language->getId();
		}
	}
	...
	public function processChunk($chunk, $contenttype, $index, &$context) {
		...
		foreach($chunk as $values)
		{
			...
			{
				if(in_array($field_name,$fieldArr)) { continue; }
				if ($node->hasField($field_name)) {
					...
					/*else if($field_type == "text_with_summary")
					{
						$default_format = $field_definition->getFieldStorageDefinition()->getSetting('default_format');
						...
						$node->{$field_name}->format = $default_format;
	...
	public function process_media_string($input_string) {
	  ...
	  public function create_media_from_url($url, $type) {
		...
		if($type != "remote_video")
		{
			...
			//echo $file->id()."<hr/>";
			if($type=="image"){  $targetField = "field_media_image"; }
			else if($type=="document"){  $targetField = "field_media_document"; }
			else if($type=="video"){  $targetField = "field_media_video_file"; }
			else if($type=="audio"){  $targetField = "field_media_audio_file"; }
			else if($type=="remote_video"){  $targetField = "field_media_document"; }
