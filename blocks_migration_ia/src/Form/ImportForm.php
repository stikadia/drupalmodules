...
class ImportForm extends FormBase
{
	...
	public function submitForm(array &$form, FormStateInterface $form_state)
	{
		...
		$contenttype = $form_state->getValue('block_type', '');
	...
	public function processChunk($chunk, $contenttype, $index, &$context)
	{
		...
		foreach ($chunk as $values) {
			if (isset($values["id"]) && $values["id"] > 0) {
				$block = BlockContent::load($values["id"]);

        if (!($block && $block->bundle() === $contenttype)) {
					$msgObj->addError(t("Block id, you are trying to update is not of selected content type."));
					break;
				}
				if (isset($values["langcode"])) {
					...
					if (!in_array($langcode, $this->allowlangs)) {
						$msgObj->addError(t("Block id, you are trying to update is not valid for langcode: " . $langcode));
						break;
					}

					$block_langcode = $block->language()->getId();

					if ($langcode != $block_langcode) {
						if (!$block->isTranslatable()) {
							$msgObj->addError(t("Block, you are trying to translate is not translatable"));
							break;
						}
						if ($block->hasTranslation($langcode)) {
							$block = $block->getTranslation($langcode);
						} else {
							$block = $block->addTranslation($langcode);
						}
					}
				}
			} else {
				$block = BlockContent::create(['type' => $contenttype]);
			}
			//die(dump($values));
			//die(dump($block));
			...
			foreach ($values as $field_name => $value) {
				...
				if ($block->hasField($field_name)) {
					...
					if ($field_type == "image" || $field_type == 'file') {
						...
						if (!empty($fidArr)) {
							$block->get($field_name)->setValue($fidArr);
						}
						...
						if ($target_entity_type == "node") {
							...
							$block->get($field_name)->setValue($refIds);
              ...
              foreach ($allValues as &$value) {
                ...
                $value = $formatted_datetime;
						...
						$block->get($field_name)->setValue($allValues);
			...
			if (isset($values["metatag"]) && trim($values["metatag"]) != '') {
				...
				if (!empty($metaValues)) {
					$block->set("field_meta_tags", serialize($metaValues));
				}
			}
