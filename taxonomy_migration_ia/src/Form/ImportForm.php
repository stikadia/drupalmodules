...
class ImportForm extends FormBase
{
	...
	public function submitForm(array &$form, FormStateInterface $form_state)
	{
		...
		$contenttype = $form_state->getValue('vocabulary', '');
	...
	public function processChunk($chunk, $contenttype, $index, &$context)
	{
		...
		foreach ($chunk as $values) {
			if (isset($values["tid"]) && $values["tid"] > 0) {
				$term = Term::load($values["tid"]);
        //die(dump($term));
				if (!($term && $term->bundle() === $contenttype)) {
					$msgObj->addError(t("Term id, you are trying to update is not of selected content type."));
					break;
				}
				if (isset($values["langcode"])) {
					...
					if (!in_array($langcode, $this->allowlangs)) {
						$msgObj->addError(t("Term id, you are trying to update is not valid for langcode: " . $langcode));
						break;
					}

					$term_langcode = $term->language()->getId();

					if ($langcode != $term_langcode) {
						if (!$term->isTranslatable()) {
							$msgObj->addError(t("Term, you are trying to translate is not translatable"));
							break;
						}
				...
				$term = Term::create(['type' => $contenttype]);
			}
			//die(dump($values));
			//die(dump($term));
			...
			foreach ($values as $field_name => $value) {
				...
				if ($term->hasField($field_name)) {
					...
					if ($field_type == "image" || $field_type == 'file') {
						...
						if (!empty($fidArr)) {
							$term->get($field_name)->setValue($fidArr);
						}
						...
						if ($target_entity_type == "node") {
							...
							$term->get($field_name)->setValue($refIds);
              ...
              foreach ($allValues as &$value) {
                ...
                $value = $formatted_datetime;
						...
						$term->get($field_name)->setValue($allValues);
			...
			if (isset($values["metatag"]) && trim($values["metatag"]) != '') {
				...
				if (!empty($metaValues)) {
					$term->set("field_meta_tags", serialize($metaValues));
				}
			}
